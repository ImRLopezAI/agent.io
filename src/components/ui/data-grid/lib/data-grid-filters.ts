...
import { safeParseFilterValue } from './data-grid-filter-schema'
...
export function getFilterFn<TData>(): FilterFn<TData> {
	return (row: Row<TData>, columnId: string, filterValue: unknown): boolean => {
		...
		const filter = safeParseFilterValue(filterValue)
		if (!filter) {
			// Malformed filter — treat as no-op so the row is included.
			return true
		}
