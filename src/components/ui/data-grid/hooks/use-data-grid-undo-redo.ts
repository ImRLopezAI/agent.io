...
function useDataGridUndoRedo<TData>({
	...
	const onUndo = React.useCallback(() => {
		...
		const message = `Undid last change (${entry.count} action${
			entry.count !== 1 ? 's' : ''
		})`
	...
	const onRedo = React.useCallback(() => {
		...
		const message = `Redid last change (${entry.count} action${
			entry.count !== 1 ? 's' : ''
		})`
		toast.success(message)
		propsRef.current.onAnnounce?.(message)
	...
	// Mod+Z / Mod+Shift+Z migrated to @tanstack/react-hotkeys (Unit 8b).
	// `ignoreInputs: true` keeps native textarea/input undo behavior intact.
	useHotkey('Mod+Z', () => onUndo(), {
