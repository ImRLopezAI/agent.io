...
import { ItalicIcon } from '#/components/editor/tiptap-icons/italic-icon'
...
export function useMark(config: UseMarkConfig) {
	...
	const state = useEditorState({
		...
		selector: ({ editor: activeEditor }) => ({
			canToggle: canToggleMark(activeEditor, type),
			isActive: isMarkActive(activeEditor, type),
		}),
	}) ?? { canToggle: false, isActive: false }
