...
const FileCornerIcon: React.FC = () => (
	<svg
		width='10'
		height='10'
		className='absolute top-0 right-0 text-border'
		viewBox='0 0 10 10'
		fill='currentColor'
		xmlns='http://www.w3.org/2000/svg'
	>
...
const ImageUploadDragArea: React.FC<ImageUploadDragAreaProps> = ({
	...
	return (
		<div
			className={cn(
				'relative cursor-pointer overflow-hidden rounded-md border border-border border-dashed px-6 py-8 text-center transition-all hover:border-input max-[480px]:px-4 max-[480px]:py-6',
...
const DropZoneContent: React.FC<{ maxSize: number; limit: number }> = ({
	...
	<>
		<div className='tiptap-image-upload-dropzone'>
			<div className='relative mx-auto mb-4 h-[57px] w-[43px]'>
				<FileIcon />
				<FileCornerIcon />
			</div>
			<div className='absolute top-8 left-1/2 flex size-8 -translate-x-1/2 items-center justify-center rounded-full bg-primary shadow-sm'>
				<CloudUploadIcon />
			</div>
		</div>
...
export const ImageUploadNode: React.FC<NodeViewProps> = (props) => {
	...
	return (
		<NodeViewWrapper
			className='my-8 outline-none [&_input[type=file]]:hidden'
			tabIndex={0}
			onClick={handleClick}
		>
			{!hasFiles && (
			...
			{hasFiles && (
				<div className='flex flex-col gap-3'>
					{fileItems.length > 1 && (
						<div className='flex items-center justify-between gap-3 font-medium text-foreground text-sm max-[480px]:flex-col max-[480px]:items-start'>
