...
function isPdfItem(item: FileUploadItem) {
	return (
				...
				return (
					<Card
						...
						className={cn(
							...
							index === 1 && 'z-10',
							isDragging &&
								'bg-popover not-dark:bg-clip-border text-foreground shadow-black/10 shadow-md dark:shadow-black/25',
						)}
						style={{
...
export function FileUpload({
	...
	const [pdfPreviews, setPdfPreviews] = React.useState<Record<string, string>>(
		{},
	)
	...
	const commitFiles = React.useCallback(
		(nextFiles: FileList | File[]) => {
			...
			for (const item of items) {
				if (!isPdfItem(item)) continue
				void renderPdfThumbnail(item.url, 96)
					.then((dataUrl) => {
						if (!dataUrl) return
						setPdfPreviews((previous) => ({ ...previous, [item.id]: dataUrl }))
					})
					.catch(() => {})
			}
		},
							...
							<FileThumbnail
								...
								previewImageUrl={
									file.type.startsWith('image/')
										? file.url
										: (pdfPreviews[file.id] ?? null)
								}
								isLoading={isPdfItem(file) && !pdfPreviews[file.id]}
								previewContent={
									!file.type.startsWith('image/') && !isPdfItem(file) ? (
										<span className='flex size-full items-center justify-center bg-muted font-semibold text-[9px] text-muted-foreground tracking-wide'>
