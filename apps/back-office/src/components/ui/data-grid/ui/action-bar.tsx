...
function ActionBar(props: ActionBarProps) {
	...
	const element = useRender({
		...
		enabled: Boolean(portalContainer && open),
		props: {
			role: 'toolbar',
...
type ActionBarItemClickHandler = NonNullable<ActionBarItemProps['onClick']>
type ActionBarItemFocusHandler = NonNullable<ActionBarItemProps['onFocus']>
type ActionBarItemKeyDownHandler = NonNullable<ActionBarItemProps['onKeyDown']>
type ActionBarItemMouseDownHandler = NonNullable<
	ActionBarItemProps['onMouseDown']
>
...
function ActionBarItem(props: ActionBarItemProps) {
	...
	const onClick = React.useCallback<ActionBarItemClickHandler>(
		(event) => {
			onClickProp?.(event)
			if (event.defaultPrevented) return
			...
			const shouldClose =
				closeOnSelect ??
				itemRef.current?.dataset.slot !== 'dropdown-menu-trigger'

			if (!itemSelectEvent.defaultPrevented && shouldClose) {
				onOpenChange?.(false)
			}
		},
		[closeOnSelect, onClickProp, onOpenChange, onSelect],
	)

	const onFocus = React.useCallback<ActionBarItemFocusHandler>(
		(event) => {
			onFocusProp?.(event)
			if (event.defaultPrevented) return
	...
	const onKeyDown = React.useCallback<ActionBarItemKeyDownHandler>(
		(event) => {
			onKeyDownProp?.(event)
			if (event.defaultPrevented) return
	...
	const onMouseDown = React.useCallback<ActionBarItemMouseDownHandler>(
		(event) => {
			onMouseDownProp?.(event)
			if (event.defaultPrevented) return
...
function ActionBarClose(props: ActionBarCloseProps) {
	const { asChild, className, onClick, children, ref, ...closeProps } = props
