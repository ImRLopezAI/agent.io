...
import {
	...
	commandPaletteUiAtom,
	useCommandPaletteOpenListener,
} from './command-palette-atoms'
...
function CommandPaletteEffects({
	inputRef,
	dispatchUi,
}: {
	inputRef: React.RefObject<HTMLInputElement | null>
	dispatchUi: (action: CommandPaletteUiAction) => void
}) {
	const prevOverflowRef = useRef<string | null>(null)

	useCommandPaletteOpenListener(
		useCallback(
			(_get, _set, newOpen, prevOpen) => {
				if (newOpen && !prevOpen) {
	...
	useCommandPaletteOpenListener(
		useCallback((_get, _set, newOpen, prevOpen) => {
			if (newOpen && !prevOpen) {
...
export function CommandPalette({
	...
	const [open, setInternalOpen] = useAtom(commandPaletteOpenAtom)
	const [ui, dispatchUi] = useAtom(commandPaletteUiAtom)

	useEffect(() => {
		if (controlledOpen === undefined) return
		setInternalOpen(controlledOpen)
	}, [controlledOpen, setInternalOpen])
	...
	useHotkey(
		{ key: shortcut.toUpperCase(), mod: true },
		(event) => {
	...
	const setActive = useCallback(
		(index: number) => {
			dispatchUi({ type: 'active/set', active: index })
			scrollActiveIntoView(index)
		},
		[dispatchUi, scrollActiveIntoView],
	)

	const filtered = useMemo(() => {
		if (!ui.query) return items
		return items.filter((it) => {
			const haystacks = [it.label, it.group ?? '', ...(it.keywords ?? [])]
			return haystacks.some((h) => fuzzyMatch(ui.query, h))
		})
	}, [items, ui.query])
	...
	const onKeyDown = (e: React.KeyboardEvent) => {
		if (e.key === 'ArrowDown') {
			e.preventDefault()
			const next = Math.min(filtered.length - 1, ui.active + 1)
			setActive(next)
		} else if (e.key === 'ArrowUp') {
			e.preventDefault()
			const next = Math.max(0, ui.active - 1)
			setActive(next)
		} else if (e.key === 'Enter') {
			e.preventDefault()
			const it = filtered[ui.active]
			if (it) {
				it.onSelect()
				setOpen(false)
			}
	...
	return createPortal(
		<>
			<CommandPaletteEffects inputRef={inputRef} dispatchUi={dispatchUi} />
			<div
				aria-hidden={!open}
				className={cn(
					'fixed inset-0 z-[100]',
			...
			>
				<motion.div
					...
					className={cn(
						'absolute inset-0 bg-background/5 backdrop-blur-12 backdrop-saturate-140% [-webkit-backdrop-filter:blur(12px)_saturate(140%)]',
						open ? 'pointer-events-auto' : 'pointer-events-none',
					)}
				/>
					...
					>
						<div className='flex items-center gap-3 border-border border-b px-4'>
							...
							<input
								...
								value={ui.query}
								onChange={(e) =>
									dispatchUi({ type: 'query/set', query: e.target.value })
								}
								...
								aria-activedescendant={
									filtered.length > 0 ? `${uid}-opt-${ui.active}` : undefined
								}
								aria-autocomplete='list'
								className='h-12 flex-1 bg-transparent text-foreground text-sm placeholder:text-muted-foreground focus-visible:outline-none'
							/>
							...
							{filtered.length === 0 ? (
								...
								grouped.map(([group, list]) => (
									<div key={group} className='mb-1 last:mb-0'>
										<div
											aria-hidden
											className='px-2 py-1.5 font-semibold text-[10px] text-muted-foreground uppercase tracking-wider'
										>
											{group}
										</div>
										{list.map((it) => {
											const idx = cursor++
											const isActive = idx === ui.active
											const Icon = it.icon
											return (
												<button
													key={it.id}
